function [model, expectations] = nppcaMaster(Y, varY, latentDim, options)

% NPPCAMASTER performs the noisy probabilistic PCA algorithm on a dataset.

% NPPCA

% Y and varY are the microarray reading and errors respectively,
%latentDim the number of latent dimensions required (although the algorithm
%may return less dimensions by automatic model selection). Options are the
%options set in the file nppcaOptions.
